...
'''
I don't think we actually need this in Skew:
Because we have a 1:1 sorted correspondence between the letters and the numbers, so when we get a SA
as output with indices, we can just lookup these indices in the original string, and they should be correct
'''
def map_ints_to_string(ints, num_to_letter_dict):
	res = []
	res_str = ""
	for i in ints:
		res.append(num_to_letter_dict[i])
		res_str += num_to_letter_dict[i]
	return res, res_str
...
def skew_rec(x, alpha_size):
	...
	new_alphabet = get_alphabet(x, sa_12)
...
#str_ls, string_2 = map_ints_to_string(ints, alpha)
#print(str_ls)
#print(string_2)
